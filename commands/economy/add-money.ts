...
export default class Command implements BaseCommand {
    ...
    public readonly pexes = [ "global.access", "commands.economy.addmoney" ];
    public readonly options = [{
        type: 6,
        name: "user",
        description: lang.setLanguage("en").get("OPT_ADDMONEY_USER_DESC"),
        required: true
    }, {
        type: 10,
        name: "money",
        description: lang.setLanguage("en").get("OPT_ADDMONEY_MONEY_DESC"),
        required: true
    }, {
        type: 3,
        name: "type",
        description: lang.setLanguage("en").get("OPT_ADDMONEY_TYPE_DESC"),
        choices: [
    ...
    public async run(interaction: CommandInteraction, guildConfig: GuildConfig) {
        ...
        if (isFloat(money) || money < 1 || money > 1000000) return await interaction.followUp({
            embeds: [sender.error(lang.get("DATA_COMMANDS_ADDMONEY_INVALIDAMOUNT"))]
        });
        ...
        await interaction.followUp({
            embeds: [sender.success(lang.get("DATA_COMMANDS_ADDMONEY_SUCCESS", [
