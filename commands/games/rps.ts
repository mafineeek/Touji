...
export default class Command implements BaseCommand {
    ...
    public readonly pexes = [ "global.access", "commands.games.rps" ];
    ...
    public async run(interaction: CommandInteraction, guildConfig: GuildConfig) {
        ...
        else if (interaction.member === user) return await interaction.followUp({
            embeds: [sender.error(lang.get("DATA_COMMANDS_RPS_WITHUS"))]
        })
        else if (["offline", "invisible"].includes((await user.fetch()).presence?.status!)) return await interaction.followUp({
            embeds: [sender.error(lang.get("DATA_COMMANDS_RPS_CANNOTPLAY"))]
        });

        const prompt = await new ConfirmationPrompt(interaction)
        .setText(lang.get("DATA_COMMANDS_RPS_WAITING", [{ old: "user", new: user.toString() }]))
