...
export default class PEXManager {
  ...
  public async has(
    ...
    const defaultResult = (
      client.helpers.getUserGroup(guild.members.cache.get(userID)!) as any
    )[pex];

    return (
      (await this.hasInGroup(userID, guildId, pex)) ??
      (await this.hasUser(userID, guildId, pex)) ??
      defaultResult ??
      true
    );
  }
  ...
  public async hasData(
    ...
    const data = (
      await r
        .table("pex-permissions")
        .getAll([userID, guildId], { index: "get" })
        .run(this.connection)
    )[0] as PEXUserData;
  ...
  public async getAll(userID: string, guildId: string): Promise<PEXUserData[]> {
    return await r
      .table("pex-permissions")
      .getAll([userID, guildId], { index: "getAll" })
      .run(this.connection);
  }
  ...
  public async inGroup(
    ...
    const response = await r
      .table("pex-groups")
      .getAll([guildId, groupName], { index: "get" })
      .run(this.connection);
  ...
  public async addToGroup(
    data: GroupData & { userID: string }
  ): Promise<boolean> {
    const groupData = (
      await r
        .table("pex-groups")
        .getAll([data.guildId, data.groupName], { index: "get" })
        .run(this.connection)
    )[0];
    return (
      (
  ...
  public async removeFromGroup(
    data: Partial<GroupData> & { userID: string }
  ): Promise<boolean> {
    const groupData = (
      await r
        .table("pex-groups")
  ...
  public async addPEXToGroup(
    data: GroupData & { pex: string }
  ): Promise<boolean> {
    const groupData = (
      await r
        .table("pex-groups")
  ...
  public async removePEXFromGroup(
    data: GroupData & { pex: string }
  ): Promise<boolean> {
    const groupData = (
      await r
  ...
  public async addPEXToUser(
    data: PEXUserData & { pex: string }
  ): Promise<boolean> {
    ...
    const userData = (
      await r
        .table("pex-permissions")
        .getAll([data.userID, data.guildId], { index: "get" })
        .run(this.connection)
    )[0];
    return (
      (
  ...
  public async removePEXFromUser(
    data: PEXUserData & { pex: string }
  ): Promise<boolean> {
    await this.ensure(data.userID, data.guildId);
  ...
  public async addGlobal(pex: string): Promise<boolean> {
    await r
      .table("pex-global-permissions")
      .get(`-${pex}`)
      .delete()
      .run(this.connection);

    const nextResponse = await r
      .table("pex-global-permissions")
      .insert({ id: pex })
      .run(this.connection);
  ...
  public async addPEXesToUser(
    data: PEXUserData & { pexes: string[] }
  ): Promise<boolean> {
    await this.ensure(data.userID, data.guildId);
  ...
  private async modifyPermissions(
    data: Partial<PEXUserData> & { pex: string }
  ): Promise<boolean> {
    const response = (await this.hasData(
      data.userID!,
      data.guildId!,
      data.pex!
    ))
  ...
  private async hasUser(
    ...
    const response = await r
      .table("pex-permissions")
      .getAll([userID, guildId], { index: "get" })
      .filter(function (row: RDatum<string>) {
        return row("pexes")
          .contains(pex)
          .or(row("pexes").contains(`-${pex}`));
      })
      .run(this.connection);

    return typeof response[0] === "undefined"
      ? undefined
      : !response[0].pexes?.includes(`-${pex}`) ||
          response[0].pexes?.includes(pex);
  }

  private async hasGlobal(
    userID: string,
    pex: string
  ): Promise<boolean | undefined> {
    const response = (
    ...
    return Config.PERMISSIONS.developer.includes(userID)
      ? true
      : typeof response === "undefined"
      ? undefined
      : !(response?.id === `-${pex}`) || response?.id === pex;
  }

  private async hasInGroup(
    userID: string,
    guildId: string,
    pex: string
  ): Promise<boolean | undefined> {
    ...
    return (
      ((response[0]?.pexes?.includes(`-${pex}`)
        ? false
        : response[0]?.pexes?.includes(pex)) &&
        response[0]?.roles?.some((rId: string) => user.roles.cache.has(rId))) ||
