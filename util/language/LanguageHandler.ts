...
export default class LanguageHandler {
  public constructor(private language: Language | "index") {}

  public static get(
    scope: string,
    variables: Array<{ old: string; new: any }>,
    language: Language
  ) {
  ...
  public get(scope: string, variables?: Array<{ old: string; new: any }>) {
    ...
    if (variables)
      for (const variable of variables) {
        raw = raw.replace(
          `{{${variable.old}}}`,
          variable.new ?? this.resolveUnknown()
        );
      }
  ...
  public getStatic(
    ...
    let raw = 
      this.setLanguage("index").rawGlobal()
    .propertiesLower();
    for (const part of scope.split("_")) {
      if (!raw.hasOwnProperty(part.toLowerCase())) return scope;
      raw = raw[part.toLowerCase()];
  ...
  public raw() {
    const data = require(`../../data/language/${this.language}.json`);
    return data
  }

  public rawGlobal() {
    const data = require(`../../data/language/global/${this.language}.json`);
    return data;
  }

  static raw(language: Language) {
    const data = require(`../../data/language/${language}.json`);
    return data;
  }

  static rawGlobal(language: Language | "index") {
    const data = require(`../../data/language/global/${language}.json`);
    return data;
  }
}
